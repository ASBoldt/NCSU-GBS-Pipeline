...
public class barcodeSorting extends AbstractPlugin {  
    static long startTime;		// reporter variable holding system time in milliseconds
    private ArgsEngine engine = null;
    private Logger logger = Logger.getLogger(barcodeSorting.class);
    ...
    protected static TreeMap<String, Integer> reporterBcTally = new TreeMap<String, Integer>();
    protected static TreeMap<String, ArrayList<String>> barcodeLog = new TreeMap<String, ArrayList<String>>();
   

    public barcodeSorting() {
        super(null, false);
    }

    public barcodeSorting(Frame parentFrame) {
...
//                        \\. escape . so it doesn't mean 'any char' & escape the backslash    
		...
		for(int read=0; read<2; read++) {
            ...
            for(int b=0;b<indexStartOfRead2;b++){
                ...
                // sortingBarcodes method, NOT NECESSARY?
         //       loadBarcodes(thePBR);
	                    ...
	                    try{
	                        ...
	                        if(currLine==idLine){
	                            ...
	                            if (rr[0] != null && rr[1] !=null){
	                                ...
	                                addIt(tempSeqF, tempSeqR, tempIdF, tempIdR);
        ...
        printBarcodeFiles(outputDir);
        
        printTally(outputDir);
    ...
    // Not necessary?
    private static void loadBarcodes(ParseBarcodeRead [] pbr){
    	String [] list1 = new String[pbr[0].getBarCodeCount()];
    	String [] list2 = new String[pbr[1].getBarCodeCount()];
    	int list1Count = pbr[0].getBarCodeCount();
    	int list2Count = pbr[1].getBarCodeCount();
    	
    	for(int i=0;i<list1Count;i++){
    		list1[i]=pbr[0].getTheBarcodes(i).getBarcode();
    	}
    	
    	for(int i=0;i<list2Count;i++){
    		list2[i]=pbr[1].getTheBarcodes(i).getBarcode();
    	}
    	    	
    	for(int i=0;i<list1Count;i++){
    		for(int j=0;j<list2Count;j++){
    			barcodeLog.put((list1[i]+"-"+list2[j]), null);
    			reporterBcTally.put((list1[i]+"-"+list2[j]), null);
    			System.out.println(list1[i]+"-"+list2[j]);
    		}
    	}
    	
    	if(barcodeLog.size()!=reporterBcTally.size()){
    		System.out.println("There is a problem loading your barcode combinations.");
    		System.exit(0);}
    	else
    		System.out.println("The number of unique barcode combinations is " + barcodeLog.size());

    }
    
    private static void addIt(String forward, String reverse, String idF, String idR){
    	String tag = forward+reverse;
    	String id1[]=idF.split(":");
    	String id2[]=idR.split(":");
    	String barcodeIDs = id1[1]+"-"+id2[1];
    	...
    	if(barcodeLog.isEmpty() && reporterBcTally.isEmpty()){
    		barcodeLog.put(barcodeIDs, temp);
    		temp.add(tag);
    		reporterBcTally.put(barcodeIDs,1);
    	}else if(!barcodeLog.containsKey(barcodeIDs)){
    		barcodeLog.put(barcodeIDs, temp);
    		temp.add(tag);
    		reporterBcTally.put(barcodeIDs,1);
    	}else if(barcodeLog.get(barcodeIDs).contains(tag))
    		;
    	else{
    		barcodeLog.get(barcodeIDs).add(tag);
    		reporterBcTally.put(barcodeIDs,1+reporterBcTally.get(barcodeIDs));
    	}
    }
    
	private static void printBarcodeFiles(String directoryInfo){

    	System.out.println("\nprintBarcodeFilesp: Printing tags to individual barcode files");
    	
    	try {
        	for(String k: barcodeLog.keySet()){
        		String key = k.toString();
        		
        		PrintWriter out = new PrintWriter(
            		new BufferedWriter(
            				new FileWriter(
            						directoryInfo+File.separator+key+".txt", false)));

        		barcodeLog.get(key).trimToSize();
        		
        		for(String value: barcodeLog.get(key)){
        			out.println(value);
        		}
        		out.close();		// close PrintWriter
        	}
	...
	private static void printTally(String directoryInfo){
		try {
        	PrintWriter out = new PrintWriter(
            		new BufferedWriter(
            				new FileWriter(
            						directoryInfo+File.separator+"TALLY.txt", false)));
        	for(String k: reporterBcTally.keySet()){
        		String key = k.toString();        		
        		int value= reporterBcTally.get(key);
        		out.println(key+"\t"+value);
           	}
        	out.close();		// close PrintWriter     
        }catch (IOException e) {
    ...
    // Next 3 override methods are necessary for class abstract extension
    @Override
